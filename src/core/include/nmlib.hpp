...
struct tableRow {
    ...
    tableRow(double xi, double vi, double yi, double v2i, double y2i, double viv2i, double LE, double LEOLE, double hi, uint C1, uint C2, double ui, double uvi)
        : xi(xi), vi(vi), yi(yi), v2i(v2i), y2i(y2i), viv2i(viv2i), LEOLE(LEOLE), LE(LE), hi(hi), C1(C1), C2(C2), ui(ui), uvi(uvi) {}
    double xi;
    double vi;
    double v2i;
    double yi;
    ...
    std::tuple<double, double, double, double, double, double, double, double, double, uint, uint, double, double> get_tuple() {
        return std::tuple<double, double, double, double, double, double, double, double, double, uint, uint, double, double>(xi,
                                                                                                                              vi,
                                                                                                                              yi,
                                                                                                                              v2i,
                                                                                                                              y2i,
    ...
    friend std::ofstream& operator<<(std::ofstream& os, const tableRow& row) {
        os << row.xi << '\t' << row.vi << '\t' << row.v2i << '\t' << row.yi << '\t' << row.y2i << '\t' << row.viv2i << '\t' << row.LE << '\t' << row.LEOLE
           << '\t' << row.hi << '\t' << row.C1 << '\t' << row.C2 << '\t' << row.ui << '\t' << row.uvi;
        return os;
    }
};
...
void dumpTableInFile(std::ofstream&, const resultTable&);